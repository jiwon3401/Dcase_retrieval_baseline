...
import audio_encoders, text_encoders
...
class DualEncoderModel(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs["audio_enc"]["init"] == "prior":
            self.audio_enc.load_state_dict(kwargs["audio_enc"]["weight"], strict=False)

            
            # Freeze or fine-tune pretrained weights
            if kwargs["audio_enc"]["name"] == "CNN14Encoder":
#                 for name, param in self.audio_enc.cnn.parameters():
#                     if name.startswith('')

                #layer 전부 다 풀어버림 - 오버피팅 안생기나?
                for param in self.audio_enc.bn0.parameters():
                    param.requires_grad = kwargs["audio_enc"].get("trainable", False)
                    #.get() -> dictionary 반환 : get("trainable",False) trainable key가 없을때 False반환?
            ...
            ###
            #elif kwargs["audio_enc"]["name"] =="Resnet38":
